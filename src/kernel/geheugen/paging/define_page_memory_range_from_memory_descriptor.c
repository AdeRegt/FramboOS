...
void define_page_memory_range_from_memory_descriptor(MemoryDescriptor *desc){
    ...
    // Align DOWN to previous 2 MiB boundary to ensure start is covered
    uint64_t aligned_start = region_start & ~(LARGE_PAGE_SIZE - 1);

    // Map from aligned start to end
    for(uint64_t addr = aligned_start; addr < region_end; addr += LARGE_PAGE_SIZE){
